...
<?php $__env->startSection('title', 'Edit Faraid Calculation'); ?>
...
<!-- Viz.js for Graphviz -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js" integrity="sha512-Mk7+95H5MxVfX8JqJxH8O7oN0QkHrQd6F+JFQ0d5q5Ff5h5H5v5Ff5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f" crossorigin="anonymous"></script>
...
<style>
    /* ===== ALL EXISTING STYLES REMAIN EXACTLY THE SAME AS IN INDEX.BLADE.PHP ===== */
    :root {
    ...
    * {
    ...
    /* Make Graphviz tree nodes more visible */
    ...
    /* Ensure family tree fits in exported reports */
    @media print {
    ...
    /* Ensure the family tree fits well in the reports */
    #graphviz-svg-container svg {
        max-width: 100%;
        height: auto;
    }

    /* Make the tree scale better for exports */
...
<div class="loading-overlay" id="loadingOverlay">
    <div class="loading-content">
        ...
        <div class="loading-subtext" id="loadingSubtext">Please wait while we update your calculation</div>
    </div>
</div>
...
<header class="hero-section calculator-header">
    ...
    <div class="container hero-container">
        <div class="hero-kicker">
            <div class="kicker-content">
                <span class="kicker-item">
                    <svg class="kicker-icon" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                    Precise
                </span>
                <span class="kicker-item">
                    <svg class="kicker-icon" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
        ...
        <h1 class="hero-title">
            <span class="hero-highlight">Edit</span> Faraid Calculation
        </h1>
        
        <p class="hero-subtitle">
            Update Islamic inheritance calculation with family tree visualization, and export capabilities
        </p>
    </div>
</header>
...
<div class="glass-container">
    ...
    <div class="modern-tabs">
        ...
        <button class="modern-tab" data-section="heirs">
            <svg class="tab-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197"/></svg>
            Heirs
        </button>
    ...
    <?php
        // Decode JSON data from the calculation object
        $heirsData = is_array($calculation->heirs_data) ? $calculation->heirs_data : (json_decode($calculation->heirs_data, true) ?: []);
        $assetsData = is_array($calculation->assets_data) ? $calculation->assets_data : (json_decode($calculation->assets_data, true) ?: []);
        $calculationData = is_array($calculation->calculation_data) ? $calculation->calculation_data : (json_decode($calculation->calculation_data, true) ?: []);
        $chartData = is_array($calculation->chart_data) ? $calculation->chart_data : (json_decode($calculation->chart_data, true) ?: []);
        $treeData = is_array($calculation->tree_data) ? $calculation->tree_data : (json_decode($calculation->tree_data, true) ?: []);
        $scenarioData = is_array($calculation->scenario_data) ? $calculation->scenario_data : (json_decode($calculation->scenario_data, true) ?: []);
        
        // Default heirs data structure
        $defaultHeirs = [
            'husband_count' => 0,
            'wife_count' => 0,
            ...
            'full_sister_count' => 0,
            'paternal_half_brother_count' => 0,
            'paternal_half_sister_count' => 0,
            'maternal_half_brother_count' => 0,
            'maternal_half_sister_count' => 0,
            'fathers_father_status' => 'deceased',
            'fathers_mother_status' => 'deceased',
            'mothers_mother_status' => 'deceased',
        ];
        $heirsData = array_merge($defaultHeirs, $heirsData);
        
        // Assets data: extract properties
        $properties = [];
        if (isset($assetsData['properties']) && is_array($assetsData['properties'])) {
            $properties = $assetsData['properties'];
        } elseif (isset($assetsData['assets']) && is_array($assetsData['assets'])) {
            $properties = $assetsData['assets'];
        }
        
        // Asset classification mapping (same as JavaScript version)
        $assetClassificationMap = [
            "Residential House" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Apartment / Condominium" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Low-cost Flat (PPR / Kos Rendah)" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Shop Lot" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Office Unit" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Industrial Property (Factory / Warehouse)" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Agricultural Land" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Vacant Land / Lot" => ["badge" => "badge-real-estate", "label" => "Real Estate"],
            "Vehicle (Car / Motorcycle)" => ["badge" => "badge-financial", "label" => "Movable Asset"],
            "Bank Savings" => ["badge" => "badge-financial", "label" => "Financial Asset"],
            "Fixed Deposit" => ["badge" => "badge-financial", "label" => "Financial Asset"],
            "EPF / KWSP Savings" => ["badge" => "badge-financial", "label" => "Financial Asset"],
            "Tabung Haji Savings" => ["badge" => "badge-financial", "label" => "Financial Asset"],
            "ASB / Unit Trust Investment" => ["badge" => "badge-investment", "label" => "Investment Asset"],
            "Shares / Stocks" => ["badge" => "badge-investment", "label" => "Investment Asset"],
            "Gold / Precious Metals" => ["badge" => "badge-investment", "label" => "Investment Asset"],
            "Business Ownership" => ["badge" => "badge-investment", "label" => "Business Asset"],
            "Insurance / Takaful Payout" => ["badge" => "badge-financial", "label" => "Financial Asset"],
            "Cash in Hand" => ["badge" => "badge-financial", "label" => "Liquid Asset"],
            "Digital Assets (Crypto / E-wallet)" => ["badge" => "badge-digital", "label" => "Digital Asset"],
            "Other Assets" => ["badge" => "badge-real-estate", "label" => "Other Asset"]
        ];
        
        // Prepare distribution summary (for results display)
        $distributionSummary = $calculation->distribution_summary ? (is_array($calculation->distribution_summary) ? $calculation->distribution_summary : json_decode($calculation->distribution_summary, true)) : [];
        if (empty($distributionSummary) && !empty($calculationData)) {
            // Build from calculation_data if needed
            $distributionSummary = [
                'heirs' => $calculationData,
                'total_distributed' => array_sum(array_column($calculationData, 'amount')),
                'total_eligible' => count($calculationData),
                'net_estate' => $calculation->net_assets,
                'total_heirs' => count($calculationData),
            ];
        }
        
        // Extract scenario number
        $scenarioNumber = $calculation->scenario_number ?? 15;
        $scenarioDescription = $calculation->scenario_description ?? 'Standard Inheritance Distribution';
    ?>
    
    <form id="faraidForm" action="<?php echo e(route('calculator.update', $calculation->id)); ?>" method="POST" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        <?php echo method_field('PUT'); ?>
        
        <input type="hidden" id="totalAssets" name="total_assets" value="<?php echo e($calculation->total_assets); ?>">
        <input type="hidden" id="deceasedNric" name="deceased_nric" value="<?php echo e($calculation->deceased_nric); ?>">
        
        <!-- SECTION 1: DECEASED -->
        <div class="glass-card active-section" id="deceasedSection">
            <div class="card-header">
                <svg class="card-header-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="text" id="deceasedName" name="deceased_name" class="form-input-modern" 
                                       placeholder="Enter full name" required value="<?php echo e(old('deceased_name', $calculation->deceased_name)); ?>">
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <select id="deceasedGender" name="deceased_gender" class="form-input-modern" required>
                                    ...
                                    <option value="male" <?php echo e((old('deceased_gender', $calculation->deceased_gender) == 'male') ? 'selected' : ''); ?>>Male</option>
                                    <option value="female" <?php echo e((old('deceased_gender', $calculation->deceased_gender) == 'female') ? 'selected' : ''); ?>>Female</option>
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="date" id="dateOfDeath" name="date_of_death" class="form-input-modern" 
                                       required value="<?php echo e(old('date_of_death', $calculation->date_of_death ? $calculation->date_of_death->format('Y-m-d') : date('Y-m-d'))); ?>">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <select id="maritalStatus" name="marital_status" class="form-input-modern" required>
                                    ...
                                    <option value="single" <?php echo e((old('marital_status', $calculation->marital_status) == 'single') ? 'selected' : ''); ?>>Single</option>
                                    <option value="married" <?php echo e((old('marital_status', $calculation->marital_status) == 'married') ? 'selected' : ''); ?>>Married</option>
                                    <option value="divorced" <?php echo e((old('marital_status', $calculation->marital_status) == 'divorced') ? 'selected' : ''); ?>>Divorced</option>
                                    <option value="widowed" <?php echo e((old('marital_status', $calculation->marital_status) == 'widowed') ? 'selected' : ''); ?>>Widowed</option>
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="text" id="causeOfDeath" name="cause_of_death" class="form-input-modern" 
                                       placeholder="Enter cause of death (if known)" required value="<?php echo e(old('cause_of_death', $calculation->cause_of_death)); ?>">
                                <svg class="input-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="text" id="deathPlace" name="death_place" class="form-input-modern" 
                                       placeholder="Enter place of death" required value="<?php echo e(old('death_place', $calculation->death_place)); ?>">
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="email" id="contactEmail" name="contact_email" class="form-input-modern" 
                                       placeholder="Enter contact email" required value="<?php echo e(old('contact_email', $calculation->contact_email)); ?>">
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <input type="tel" id="contactPhone" name="contact_phone" class="form-input-modern" 
                                       placeholder="Enter contact phone number" required value="<?php echo e(old('contact_phone', $calculation->contact_phone)); ?>">
                        ...
                        <div class="form-group-modern">
                            ...
                            <div class="input-group-modern">
                                <textarea id="residentialAddress" name="residential_address" class="form-input-modern" 
                                          rows="2" placeholder="Enter full residential address" required><?php echo e(old('residential_address', $calculation->residential_address)); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="glass-card" id="assetsSection">
            <div class="card-header">
                <svg class="card-header-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                <h2 class="card-title">Assets</h2>
                <span class="card-badge" id="totalAssetsBadge"><?php echo e(count($properties)); ?> <?php echo e(count($properties) == 1 ? 'Asset' : 'Assets'); ?></span>
            </div>
            ...
            <div class="card-body">
                ...
                <div class="summary-grid">
                    <div class="summary-card">
                        ...
                        <div class="summary-value" id="totalAssetsGrossDisplay">RM <?php echo e(number_format(array_sum(array_column($properties, 'value')), 2)); ?></div>
                        <div class="summary-subtitle">Sum of all properties</div>
                    </div>
                    
                    <div class="summary-card">
                        <div class="summary-title">Total Assets (Your Share)</div>
                        <div class="summary-value" id="totalAssetsValue">RM <?php echo e(number_format(array_sum(array_map(function($p) { return $p['value'] * ($p['ownership'] ?? 100) / 100; }, $properties)), 2)); ?></div>
                        <div class="summary-subtitle">After ownership percentage</div>
                    </div>
                    
                    <div class="summary-card">
                        <div class="summary-title">Eligible for Distribution</div>
                        <div class="summary-value text-success" id="netAssets">RM <?php echo e(number_format($calculation->net_assets, 2)); ?></div>
                ...
                <div class="category-section">
                    ...
                    <div class="btn-group-modern" style="margin-top: 1rem; padding-top: 0; border-top: 0;">
                        <button type="button" class="btn btn-primary" id="addPropertyBtn">
                            <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                            Add Asset
                        </button>
                        <button type="button" class="btn btn-danger" id="clearPropertiesBtn">
                            <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            Clear All
                        </button>
                    </div>
                            ...
                            <table class="results-table">
                                ...
                                <tbody id="propertiesList">
                                    <?php if(count($properties) > 0): ?>
                                        <?php $__currentLoopData = $properties; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $prop): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                                $propValue = $prop['value'] ?? 0;
                                                $ownership = $prop['ownership'] ?? 100;
                                                $share = $propValue * $ownership / 100;
                                                $classification = $assetClassificationMap[$prop['type'] ?? 'Other Assets'] ?? ['badge' => 'badge-real-estate', 'label' => 'Asset'];
                                            ?>
                                            <tr>
                                                <td><strong><?php echo e($prop['type'] ?? ''); ?></strong></td>
                                                <td><span class="badge <?php echo e($classification['badge']); ?>"><?php echo e($classification['label']); ?></span></td>
                                                <td><?php echo e($prop['description'] ?? '-'); ?></span></td>
                                                <td><strong>RM <?php echo e(number_format($propValue, 2)); ?></strong></td>
                                                <td><?php echo e($ownership); ?>%</span></td>
                                                <td><strong>RM <?php echo e(number_format($share, 2)); ?></strong></td>
                                                <td><button type="button" class="btn btn-sm btn-danger delete-property" data-index="<?php echo e($index); ?>">Remove</button></td>
                                            </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="7" class="text-center py-8 text-muted">No assets added yet</span>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                                <tfoot id="propertiesTotal" style="<?php echo e(count($properties) > 0 ? '' : 'display: none;'); ?>">
                                    <tr style="background: var(--primary-light); font-weight: 600;">
                                        <td colspan="5" style="text-align: right;">Total Assets:</span>
                                        <td id="propertiesTotalValue" colspan="2">RM <?php echo e(number_format(array_sum(array_map(function($p) { return $p['value'] * ($p['ownership'] ?? 100) / 100; }, $properties)), 2)); ?></span>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <div class="btn-group-modern">
                    <button type="button" class="btn btn-secondary prev-section" data-prev="deceased">
                        <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                        Back
                    </button>
        ...
        <div class="glass-card" id="heirsSection">
            <div class="card-header">
                <svg class="card-header-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197"/></svg>
                    ...
                    <div class="heir-grid">
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="husbandCount" name="husband_count" class="counter-input" 
                                       value="<?php echo e($heirsData['husband_count'] ?? 0); ?>" min="0" max="1" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="wifeCount" name="wife_count" class="counter-input" 
                                       value="<?php echo e($heirsData['wife_count'] ?? 0); ?>" min="0" max="4" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="status-select">
                                <button type="button" class="status-btn <?php echo e(($heirsData['father_status'] ?? 'deceased') == 'alive' ? 'active' : ''); ?>" data-target="fatherStatus" data-value="alive">Alive</button>
                                <button type="button" class="status-btn <?php echo e(($heirsData['father_status'] ?? 'deceased') == 'deceased' ? 'active' : ''); ?>" data-target="fatherStatus" data-value="deceased">Deceased</button>
                            </div>
                            <input type="hidden" id="fatherStatus" name="father_status" value="<?php echo e($heirsData['father_status'] ?? 'deceased'); ?>">
                        </div>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="counter-hint">Status</div>
                            <div class="status-select">
                                <button type="button" class="status-btn <?php echo e(($heirsData['mother_status'] ?? 'deceased') == 'alive' ? 'active' : ''); ?>" data-target="motherStatus" data-value="alive">Alive</button>
                                <button type="button" class="status-btn <?php echo e(($heirsData['mother_status'] ?? 'deceased') == 'deceased' ? 'active' : ''); ?>" data-target="motherStatus" data-value="deceased">Deceased</button>
                            </div>
                            <input type="hidden" id="motherStatus" name="mother_status" value="<?php echo e($heirsData['mother_status'] ?? 'deceased'); ?>">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="sonCount" name="son_count" class="counter-input" 
                                       value="<?php echo e($heirsData['son_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="daughterCount" name="daughter_count" class="counter-input" 
                                       value="<?php echo e($heirsData['daughter_count'] ?? 0); ?>" min="0" readonly>
                ...
                <div class="category-section">
                    ...
                    <div class="heir-grid">
                        <div class="heir-card">
                            ...
                            <div class="status-select">
                                <button type="button" class="status-btn <?php echo e(($heirsData['fathers_father_status'] ?? 'deceased') == 'alive' ? 'active' : ''); ?>" data-target="fathersFatherStatus" data-value="alive">Alive</button>
                                <button type="button" class="status-btn <?php echo e(($heirsData['fathers_father_status'] ?? 'deceased') == 'deceased' ? 'active' : ''); ?>" data-target="fathersFatherStatus" data-value="deceased">Deceased</button>
                            </div>
                            <input type="hidden" id="fathersFatherStatus" name="fathers_father_status" value="<?php echo e($heirsData['fathers_father_status'] ?? 'deceased'); ?>">
                        </div>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="status-select">
                                <button type="button" class="status-btn <?php echo e(($heirsData['fathers_mother_status'] ?? 'deceased') == 'alive' ? 'active' : ''); ?>" data-target="fathersMotherStatus" data-value="alive">Alive</button>
                                <button type="button" class="status-btn <?php echo e(($heirsData['fathers_mother_status'] ?? 'deceased') == 'deceased' ? 'active' : ''); ?>" data-target="fathersMotherStatus" data-value="deceased">Deceased</button>
                            </div>
                            <input type="hidden" id="fathersMotherStatus" name="fathers_mother_status" value="<?php echo e($heirsData['fathers_mother_status'] ?? 'deceased'); ?>">
                        </div>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="status-select">
                                <button type="button" class="status-btn <?php echo e(($heirsData['mothers_mother_status'] ?? 'deceased') == 'alive' ? 'active' : ''); ?>" data-target="mothersMotherStatus" data-value="alive">Alive</button>
                                <button type="button" class="status-btn <?php echo e(($heirsData['mothers_mother_status'] ?? 'deceased') == 'deceased' ? 'active' : ''); ?>" data-target="mothersMotherStatus" data-value="deceased">Deceased</button>
                            </div>
                            <input type="hidden" id="mothersMotherStatus" name="mothers_mother_status" value="<?php echo e($heirsData['mothers_mother_status'] ?? 'deceased'); ?>">
                        </div>
                    </div>
                </div>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="fullBrotherCount" name="full_brother_count" class="counter-input" 
                                       value="<?php echo e($heirsData['full_brother_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="fullSisterCount" name="full_sister_count" class="counter-input" 
                                       value="<?php echo e($heirsData['full_sister_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="paternalHalfBrotherCount" name="paternal_half_brother_count" class="counter-input" 
                                       value="<?php echo e($heirsData['paternal_half_brother_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="paternalHalfSisterCount" name="paternal_half_sister_count" class="counter-input" 
                                       value="<?php echo e($heirsData['paternal_half_sister_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="maternalHalfBrotherCount" name="maternal_half_brother_count" class="counter-input" 
                                       value="<?php echo e($heirsData['maternal_half_brother_count'] ?? 0); ?>" min="0" readonly>
                        ...
                        <div class="heir-card">
                            ...
                            <div class="heir-counter">
                                ...
                                <input type="number" id="maternalHalfSisterCount" name="maternal_half_sister_count" class="counter-input" 
                                       value="<?php echo e($heirsData['maternal_half_sister_count'] ?? 0); ?>" min="0" readonly>
        ...
        <div class="glass-card" id="resultsSection">
            ...
            <div class="card-body">
                ...
                <!-- Results Content -->
                <div id="resultsContent" style="<?php echo e(!empty($distributionSummary['heirs']) ? 'display: block;' : 'display: none;'); ?>">
                    <!-- Results Summary -->
                    <div class="summary-grid">
                        <div class="summary-card">
                            <div class="summary-title">Total Heirs</div>
                            <div class="summary-value" id="resultTotalHeirs"><?php echo e($distributionSummary['total_eligible'] ?? 0); ?></div>
                            <div class="summary-subtitle">Eligible for inheritance</div>
                        </div>
                        
                        <div class="summary-card">
                            <div class="summary-title">Total Estate</div>
                            <div class="summary-value" id="resultNetEstate">RM <?php echo e(number_format($calculation->net_assets, 2)); ?></div>
                            <div class="summary-subtitle">Available for distribution</div>
                        </div>
                        
                        <div class="summary-card">
                            <div class="summary-title">Scenario</div>
                            <div class="summary-value" id="resultScenario"><?php echo e($scenarioNumber); ?></div>
                    ...
                    <div class="category-section">
                        <div class="category-header">
                            ...
                            <span class="category-badge" id="scenarioBadge">Scenario <?php echo e($scenarioNumber); ?></span>
                        </div>
                        <div id="scenarioDescription" style="margin-top: 1rem; padding: 1.5rem; background: rgba(26, 95, 180, 0.05); border-radius: var(--border-radius-md);">
                            <p id="scenarioText" style="margin: 0; color: var(--gray-700); line-height: 1.6;"><?php echo e($scenarioDescription); ?></p>
                        </div>
                    </div>
                        ...
                        <div class="results-container">
                            <table class="results-table">
                                ...
                                <tbody id="distributionResults">
                                    <?php
                                        $heirsList = $distributionSummary['heirs'] ?? [];
                                        $totalDistributed = 0;
                                        $totalPercentage = 0;
                                    ?>
                                    <?php if(count($heirsList) > 0): ?>
                                        <?php $__currentLoopData = $heirsList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $heir): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                                ...
                                                $percentage = $heir['percentage'] ?? ($calculation->net_assets > 0 ? ($amount / $calculation->net_assets) * 100 : 0);
                                                $totalDistributed += $amount;
                                                $totalPercentage += $percentage;
                                                $fractionDisplay = $heir['fraction'] ?? ($heir['share'] ?? '0');
                                            ...
                                            ?>
                                            <tr>
                                                <td><strong><?php echo e($heir['heir'] ?? $heir['name'] ?? 'Unknown'); ?></strong></td>
                                                <td><?php echo e(ucfirst($heir['relationship'] ?? 'Unknown')); ?></span></td>
                                                <td><?php echo e($fractionDisplay); ?></span></td>
                                                <td><strong>RM <?php echo e(number_format($amount, 2)); ?></strong></span></td>
                                                <td><strong><?php echo e(number_format($percentage, 2)); ?>%</strong></span></td>
                                                <td><span class="badge <?php echo e($badgeClass); ?>"><?php echo e($status); ?></span></span></td>
                                            </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="6" class="text-center py-8 text-muted">Complete all sections and calculate results</span>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                                <?php if(count($heirsList) > 0): ?>
                                <tfoot id="distributionTotal">
                                    <tr class="total-row">
                                        <td colspan="3" style="text-align: right; color: white !important;">Total Distributed:</span>
                                        <td id="totalDistributedValue" style="color: white !important;">RM <?php echo e(number_format($totalDistributed, 2)); ?></span>
                                        <td id="totalPercentageValue" style="color: white !important;"><?php echo e(number_format($totalPercentage, 2)); ?>%</span>
                                        <td style="color: white !important;"></span>
                                    </tr>
                                </tfoot>
                                <?php endif; ?>
                            </table>
                    ...
                    <div class="family-tree-section" id="familyTreeSection">
                        <div class="tree-controls">
                            ...
                            <div class="tree-actions">
                                <!-- Generate button removed – tree is generated automatically -->
                            </div>
                        </div>
                            ...
                            <div class="chart-container">
                                <div class="pie-chart-container">
                                    ...
                                    <div class="chart-center">
                                        ...
                                        <div class="chart-center-value" id="eligibleCount"><?php echo e($distributionSummary['total_eligible'] ?? 0); ?></div>
                                ...
                                <div class="chart-legend" id="chartLegend">
                                    <?php
                                        $eligibleHeirsChart = array_filter($heirsList, function($h) { return ($h['amount'] ?? 0) > 0 && ($h['status'] ?? '') !== 'Surplus'; });
                                        $chartColors = ['#7e1ab4', '#2d7ad6', '#25D366', '#ffd700', '#dc3545', '#2eaec2', '#1a5fb4', '#ff6b6b', '#51cf66', '#9d4edd'];
                                        $colorIndex = 0;
                                    ?>
                                    <?php if(count($eligibleHeirsChart) > 0): ?>
                                        <?php $__currentLoopData = $eligibleHeirsChart; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $heir): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                                $amount = $heir['amount'] ?? 0;
                                                $percentage = $heir['percentage'] ?? ($calculation->net_assets > 0 ? ($amount / $calculation->net_assets) * 100 : 0);
                                                ...
                                                $colorIndex++;
                                            ?>
                                            <div class="legend-item" data-index="<?php echo e($loop->index); ?>">
                                                <div class="legend-color" style="background: <?php echo e($color); ?>;"></div>
                                                <div class="legend-text">
                                                    <?php echo e($heir['heir'] ?? $heir['name'] ?? 'Unknown'); ?>

                                                    <div class="legend-relationship"><?php echo e(ucfirst($heir['relationship'] ?? 'Unknown')); ?></div>
                                                    <div class="legend-amount">RM <?php echo e(number_format($amount, 2)); ?></div>
                                                </div>
                                                <div class="legend-percentage"><?php echo e(number_format($percentage, 2)); ?>%</div>
                                            </div>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        <div class="legend-summary">
                                            <div class="legend-item">
                                                <div class="legend-color" style="background: var(--primary-color);"></div>
                                                <div class="legend-text"><strong>Total Eligible Distribution</strong><div class="legend-amount"><?php echo e(count($eligibleHeirsChart)); ?> recipient(s)</div></div>
                                                <div class="legend-percentage">RM <?php echo e(number_format(array_sum(array_column($eligibleHeirsChart, 'amount')), 2)); ?></div>
                                            </div>
                                        </div>
                                    <?php else: ?>
                                        <div class="no-eligible-recipients">
                                            ...
                                            <p>No eligible recipients with positive inheritance amounts</p>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                    ...
                    <div class="export-controls">
                        ...
                        <div class="export-buttons">
                            <button type="button" class="export-btn full-report" id="exportFullReport">
                                <svg class="export-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                Export Report
                            </button>
                            <button type="button" class="export-btn tree-pdf" id="exportTreePDF">
                                <svg class="export-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                Export Family Tree
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="btn-group-modern">
                        ...
                        <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                            <button type="submit" class="btn btn-primary" id="saveCalculation">
                                Update Calculation
                            </button>
                            <button type="button" class="btn btn-secondary" id="resetCalculator">
                                Reset Changes
                            </button>
                            <a href="<?php echo e(route('calculator.history')); ?>" class="btn btn-ghost">
                                <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                                Back to List
                            </a>
                        </div>
        ...
        <!-- Hidden Fields -->
        <input type="hidden" id="heirsData" name="heirs_data" value="<?php echo e(json_encode($heirsData)); ?>">
        <input type="hidden" id="assetsData" name="assets_data" value="<?php echo e(json_encode(['properties' => $properties])); ?>">
        <input type="hidden" id="calculationData" name="calculation_data" value="<?php echo e(json_encode($calculationData)); ?>">
        <input type="hidden" id="scenarioData" name="scenario_data" value="<?php echo e(json_encode($scenarioData)); ?>">
        <input type="hidden" id="chartData" name="chart_data" value="<?php echo e(json_encode($chartData)); ?>">
        <input type="hidden" id="treeData" name="tree_data" value="<?php echo e(json_encode($treeData)); ?>">
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    // ===== GLOBAL VARIABLES =====
    let properties = <?php echo json_encode($properties, 15, 512) ?>;
    let calculationResults = <?php echo json_encode($calculationData, 15, 512) ?>;
    let detectedScenario = <?php echo e($scenarioNumber); ?>;
    let pieChart = null;
    ...
    // ===== FARAID CALCULATION - COMPLETE RULES (same as index.blade.php) =====
    ...
    function performFaraidCalculation(heirs, netEstate) {
        ...
        if (hasSpouse && hasChildren && !hasParents) {
            // SPECIAL: 2 WIVES + 2 DAUGHTERS (No sons, no other heirs)
            ...
            // Original spouse & children logic
        ...
        if (!hasSpouse && !hasParents && hasChildren && !hasSiblings) {
            if (heirs.son_count === 1 && heirs.daughter_count === 0) {
                ...
                const totalShares = (2 * 2) + 2; // 2 sons * 2 + 2 daughters = 6
        ...
        // ===== FULL SIBLINGS ONLY (NO PARENTS, NO CHILDREN, NO SPOUSE) =====
    ...
    function updateHiddenFields(formData, results, scenario, assetsSummary) {
        ...
        const calculationData = results.map(r => ({
            heir: r.heir, relationship: r.relationship, fraction: r.fractionDisplay,
            amount: r.amount, percentage: r.percentage, status: r.status
        }));
    ...
    function displayResults(results, scenario, netEstate) {
        ...
        if (results.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="text-center py-8 text-muted">No distribution calculated</span></tr>';
        ...
        results.forEach((result) => {
            ...
            row.innerHTML = `
                ...
                <td>${escapeHtml(fractionDisplay)}</span></td>
                <td><strong>RM ${amountNum.toLocaleString('en-US', {minimumFractionDigits: 2})}</strong></span></td>
                <td><strong>${parseFloat(result.percentage).toFixed(2)}%</strong></span></td>
                <td><span class="badge ${badgeClass}">${escapeHtml(result.status)}</span></span></td>
            `;
            tbody.appendChild(row);
        });
    ...
    `;
        ...
        if (heirs.fathers_father_status === 'alive') {
            ...
            if (heirs.father_status === 'alive') dot += `Father -> FathersFather [label="Parent", dir=back, fontcolor="#999"];\n`;
            else dot += `Deceased -> FathersFather [label="Grandfather", style=dashed, fontcolor="#999"];\n`;
        }
        if (heirs.fathers_mother_status === 'alive') {
            ...
            if (heirs.father_status === 'alive') dot += `Father -> FathersMother [label="Parent", dir=back, fontcolor="#999"];\n`;
            else dot += `Deceased -> FathersMother [label="Grandmother", style=dashed, fontcolor="#999"];\n`;
        }
        if (heirs.mothers_mother_status === 'alive') {
            ...
            if (heirs.mother_status === 'alive') dot += `Mother -> MothersMother [label="Parent", dir=back, fontcolor="#999"];\n`;
            else dot += `Deceased -> MothersMother [label="Grandmother", style=dashed, fontcolor="#999"];\n`;
        }
    ...
    document.getElementById('faraidForm')?.addEventListener('submit', function(e) {
        if (e.submitter && e.submitter.id === 'saveCalculation') {
            // Ensure hidden fields are updated with latest data before submit
            if (calculationResults) {
                ...
                const formData = {
                    ...
                    marital_status: document.getElementById('maritalStatus').value,
                    heirs_data: heirsData,
                    assets_data: { properties: properties, totalAssets: assetsSummary.totalAssets, netAssets: assetsSummary.netAssets }
                };
                updateHiddenFields(formData, calculationResults, detectedScenario, assetsSummary);
            }
            return true;
        }
    });
    ...
    document.getElementById('resetCalculator')?.addEventListener('click', function() {
        if (confirm('Reset all changes? Reload the page to revert to saved data.')) {
    ...
    // If there are existing results, generate the initial family tree
    if (calculationResults && calculationResults.length > 0) {
        setTimeout(() => {
            generateGraphvizTree();
        }, 500);
