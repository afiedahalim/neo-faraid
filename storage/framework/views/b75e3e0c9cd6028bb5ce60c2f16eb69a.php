...
<?php
    ...
    $isEditing = isset($estate) && $estate instanceof \App\Models\EstatePreRegistration;

    // ===== SMART DATA EXTRACTION (ONLY NEEDED FIELDS, LIMIT TO 100 EACH) =====
    $heirs = [];
    $assets = [];
    $debts = [];
    $wasiyyah = [];
    ...
    if ($isEditing) {
        // Load only the first 100 of each relation (to prevent memory exhaustion)
        $heirs = $estate->heirs()->limit(100)->get()->map(function ($h) {
            return [
                'id' => $h->id,
                'name' => $h->name,
                'nric' => $h->nric,
                ...
                'relationshipDisplay' => $h->relationship_display ?? $h->relationship,
                'percentage' => (float) $h->share_percentage,
                'phone' => $h->phone,
            ];
        })->toArray();

        $assets = $estate->assets()->limit(100)->get()->map(function ($a) {
            return [
                'id' => $a->id,
                'name' => $a->name,
                'value' => (float) $a->value,
                'ownership' => (float) ($a->ownership_percentage ?? 100),
                'description' => $a->description,
                'category' => $a->category ?? 'Other Asset',
                'badge' => $a->badge ?? 'badge-real-estate',
                'label' => $a->label ?? 'Other Asset',
            ];
        })->toArray();

        $debts = $estate->debts()->limit(100)->get()->map(function ($d) {
            return [
                'id' => $d->id,
                'name' => $d->debt_type ?? $d->name,
                'amount' => (float) $d->amount,
                'creditor' => $d->creditor_name ?? $d->creditor,
                'description' => $d->description,
                'type' => $d->type ?? 'Other Debt',
                'badge' => $d->badge ?? 'badge-unsecured',
                'label' => $d->label ?? 'Other Debt',
            ];
        })->toArray();

        $wasiyyah = $estate->wasiyyah()->limit(100)->get()->map(function ($w) {
            return [
                'id' => $w->id,
        ...
        $credentials = $estate->digitalCredentials()->limit(100)->get()->map(function ($c) {
            return [
                'id' => $c->id,
                'platform' => $c->platform,
                'username' => $c->username,
                'password' => $c->password, // encrypted, but keep as is
                'security' => $c->security_questions ?? '',
                'notes' => $c->notes ?? '',
            ];
        })->toArray();
    }

    $actionRoute = $isEditing ? route('estate-setup.update', $estate->unique_id) : route('estate-setup.store');
    $formMethod = $isEditing ? 'PUT' : 'POST';
    $pageTitle = $isEditing ? 'Edit Estate Planning' : 'Create Estate Planning';
    $user = Auth::user();

    // Helper for YouTube ID (declared ONCE)
...
(function() {
    ...
    let nextCredentialId = credentials.length ? Math.max(...credentials.map(c => c.id)) + 1 : 1;
    let nextWasiyyahId = wasiyyah.length ? Math.max(...wasiyyah.map(w => w.id)) + 1 : 1;
    ...
    // Render all lists using the data loaded from the server
    renderCredentialsList();
    renderAssetsList();
    renderDebtsList();
    renderHeirsList();
    ...
    // Set video option based on editing state
    if (currentVideoOption !== 'none') {
        setVideoOption(currentVideoOption);
        if (currentVideoOption === 'upload' && videoFile) {
            // If there's a video file (uploaded), we already have it in memory
            // The preview is shown via the HTML conditional block
        } else if (currentVideoOption === 'youtube' && youtubeUrl) {
            // The YouTube input and iframe are already populated via PHP
        }
    } else {
        setVideoOption('upload');
    }
