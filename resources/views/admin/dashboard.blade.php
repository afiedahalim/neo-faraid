...
<body>
    ...
    <nav class="admin-topbar" role="navigation" aria-label="Admin navigation">
        ...
        <div class="topbar-right">
            <a href="{{ route('profile.edit') }}" class="user-profile-btn" title="Profile Settings">
            ...
            <div class="header-content">
                ...
                <div class="quick-stats-row">
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalUsers ?? 0)) }}</span>
                        <span class="quick-stat-label">Total Users</span>
                    </div>
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalFeedback ?? 0)) }}</span>
                        <span class="quick-stat-label">Feedback</span>
                    </div>
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalFaqs ?? 0)) }}</span>
                        <span class="quick-stat-label">FAQs</span>
                    </div>
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalCalculations ?? 0)) }}</span>
                        <span class="quick-stat-label">Calculations</span>
                    </div>
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalInstantSessions ?? 0)) }}</span>
                        <span class="quick-stat-label">Instant Estate</span>
                    </div>
                    <div class="quick-stat">
                        <span class="quick-stat-value">{{ number_format((int)($totalEstateSetup ?? 0)) }}</span>
        ...
        <div class="dashboard-container">
            ...
            <div class="management-nav">
                ...
                <div class="management-nav-grid">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="management-nav-item {{ request()->is('admin/users*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-users">
                            <i class="fas fa-users"></i>
                        </div>
                        <span class="nav-item-label">Users</span>
                        <span class="nav-item-badge blue">{{ number_format((int)($totalUsers ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 2. Feedback Management --}}
                    @if(Route::has('admin.feedback.index'))
                    <a href="{{ route('admin.feedback.index') }}" class="management-nav-item {{ request()->is('admin/feedback*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-feedback">
                            <i class="fas fa-comment-dots"></i>
                        </div>
                        <span class="nav-item-label">Feedback</span>
                        <span class="nav-item-badge yellow">{{ number_format((int)($totalFeedback ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 3. FAQ Management --}}
                    @if(Route::has('admin.faq.index'))
                    <a href="{{ route('admin.faq.index') }}" class="management-nav-item {{ request()->is('admin/faq*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-faq">
                            <i class="fas fa-question-circle"></i>
                        </div>
                        <span class="nav-item-label">FAQ</span>
                        <span class="nav-item-badge green">{{ number_format((int)($totalFaqs ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 4. Calculations Management --}}
                    @if(Route::has('admin.calculations.index'))
                    <a href="{{ route('admin.calculations.index') }}" class="management-nav-item {{ request()->is('admin/calculations*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-calculations">
                            <i class="fas fa-calculator"></i>
                        </div>
                        <span class="nav-item-label">Calculations</span>
                        <span class="nav-item-badge purple">{{ number_format((int)($totalCalculations ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 5. Instant Estate Management --}}
                    @if(Route::has('admin.instant-estate.index'))
                    <a href="{{ route('admin.instant-estate.index') }}" class="management-nav-item {{ request()->is('admin/instant-estate*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-instant">
                            <i class="fas fa-bolt"></i>
                        </div>
                        <span class="nav-item-label">Instant Estate</span>
                        <span class="nav-item-badge red">{{ number_format((int)($totalInstantSessions ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 6. Estate Setup Management --}}
                    @if(Route::has('admin.estate-setup.index'))
                    <a href="{{ route('admin.estate-setup.index') }}" class="management-nav-item {{ request()->is('admin/estate-setup*') ? 'active' : '' }}">
                        <div class="nav-icon-wrapper nav-icon-estate">
                            <i class="fas fa-home"></i>
                        </div>
                        <span class="nav-item-label">Estate Setup</span>
                        <span class="nav-item-badge orange">{{ number_format((int)($totalEstateSetup ?? 0)) }}</span>
                    </a>
                    @endif

                    {{-- 7. Faraid Rules Management --}}
                    @if(Route::has('database.seeders.FaraidCalculationRulesSeeder'))
                    <a href="{{ route('database.seeders.FaraidCalculationRulesSeeder') }}" class="management-nav-item {{ request()->is('admin/faraid-rules*') ? 'active' : '' }}">
            ...
            <div class="stats-cards-grid">
                {{-- Users Card --}}
                <div class="stat-card card-users" onclick="window.location.href='{{ Route::has('admin.users.index') ? route('admin.users.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-users"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalUsers = $totalUsers ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalUsers) }}</div>
                        <div class="stat-label-text">Registered Users</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Admins: {{ number_format((int)($adminCount ?? 0)) }}</span>
                            <span class="stat-sub-item">Active: {{ number_format((int)($activeUserCount ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.users.index'))
                        <a href="{{ route('admin.users.index') }}" class="stat-link-arrow">
                ...
                {{-- Feedback Card --}}
                <div class="stat-card card-feedback" onclick="window.location.href='{{ Route::has('admin.feedback.index') ? route('admin.feedback.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-star"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalFeedback = $totalFeedback ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalFeedback) }}</div>
                        <div class="stat-label-text">Total Feedback</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Rejected: {{ number_format((int)($rejectedFeedback ?? 0)) }}</span>
                            <span class="stat-sub-item">Approved: {{ number_format((int)($approvedFeedback ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.feedback.index'))
                        <a href="{{ route('admin.feedback.index') }}" class="stat-link-arrow">
                ...
                {{-- FAQ Card --}}
                <div class="stat-card card-faq" onclick="window.location.href='{{ Route::has('admin.faq.index') ? route('admin.faq.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-question-circle"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalFaqs = $totalFaqs ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalFaqs) }}</div>
                        <div class="stat-label-text">FAQ Entries</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Published: {{ number_format((int)($publishedFaqs ?? 0)) }}</span>
                            <span class="stat-sub-item">Categories: {{ number_format((int)($faqCategories ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.faq.index'))
                        <a href="{{ route('admin.faq.index') }}" class="stat-link-arrow">
                ...
                {{-- Calculations Card --}}
                <div class="stat-card card-calculations" onclick="window.location.href='{{ Route::has('admin.calculations.index') ? route('admin.calculations.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-calculator"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalCalculations = $totalCalculations ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalCalculations) }}</div>
                        <div class="stat-label-text">Faraid Calculations</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Total Calculation: {{ number_format((int)($totalCalculations ?? 0)) }}</span>
                            <span class="stat-sub-item">Total Asset: {{ number_format((int)($totalAssets ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.calculations.index'))
                        <a href="{{ route('admin.calculations.index') }}" class="stat-link-arrow">
                ...
                {{-- Instant Estate Card --}}
                <div class="stat-card card-instant" onclick="window.location.href='{{ Route::has('admin.instant-estate.index') ? route('admin.instant-estate.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-bolt"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalInstantSessions = $totalInstantSessions ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalInstantSessions) }}</div>
                        <div class="stat-label-text">Instant Sessions</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Completed: {{ number_format((int)($completedSessions ?? 0)) }}</span>
                            <span class="stat-sub-item">Processing: {{ number_format((int)($processingSessions ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.instant-estate.index'))
                        <a href="{{ route('admin.instant-estate.index') }}" class="stat-link-arrow">
                ...
                {{-- Estate Setup Card --}}
                <div class="stat-card card-estate" onclick="window.location.href='{{ Route::has('admin.estate-setup.index') ? route('admin.estate-setup.index') : '#' }}'">
                    <div class="stat-icon-circle">
                        <i class="fas fa-home"></i>
                    </div>
                    <div class="stat-info">
                        @php $totalEstateSetup = $totalEstateSetup ?? 0; @endphp
                        <div class="stat-number">{{ number_format((int)$totalEstateSetup) }}</div>
                        <div class="stat-label-text">Pre-Registrations</div>
                        <div class="stat-sub-items">
                            <span class="stat-sub-item">Activated: {{ number_format((int)($activatedEstates ?? 0)) }}</span>
                            <span class="stat-sub-item">Draft: {{ number_format((int)($draftEstates ?? 0)) }}</span>
                        </div>
                        @if(Route::has('admin.estate-setup.index'))
                        <a href="{{ route('admin.estate-setup.index') }}" class="stat-link-arrow">
            ...
            <div class="activity-grid">
                            ...
                            @foreach($users->take(8) as $user)
                                <div class="activity-item">
                                    ...
                                    <div class="activity-meta">
                                        ...
                                        <span class="activity-badge {{ ($user->role ?? 'user') === 'admin' ? 'badge-primary' : 'badge-info' }}">
                ...
                <div class="activity-card">
                    ...
                    <div class="activity-card-body">
                        @php $recentFeedback = $recentFeedback ?? collect(); @endphp
                        @if($recentFeedback->count() > 0)
                            @foreach($recentFeedback->take(8) as $feedback)
                                <div class="activity-item">
                                    <div class="activity-left">
                                        <div class="activity-avatar avatar-purple">
                                            {{ strtoupper(substr(optional($feedback->user)->name ?? 'A', 0, 1)) }}
                                        </div>
                                        <div class="activity-details">
                                            <h4>{{ optional($feedback->user)->name ?? 'Anonymous User' }}</h4>
                                            <p>{{ \Illuminate\Support\Str::limit($feedback->message ?? 'No message', 60) }}</p>
                                        </div>
                                    </div>
                                    <div class="activity-meta">
                                        <span class="activity-time">
                                            @if(isset($feedback->created_at) && is_object($feedback->created_at))
                                        ...
                                        @endphp
                                        <span class="activity-badge {{ $fbBadgeClass }}">
                                            {{ ucfirst($fbStatus) }}
                                        </span>
                                    </div>
                                </div>
                ...
                <div class="activity-card">
                    <div class="activity-card-header">
                        ...
                        @if(Route::has('admin.calculations.index'))
                        <a href="{{ route('admin.calculations.index') }}" class="btn btn-sm btn-primary">View All</a>
                        @endif
                    </div>
                    <div class="activity-card-body">
                        @php $recentCalculations = $recentCalculations ?? collect(); @endphp
                        @if($recentCalculations->count() > 0)
                            @foreach($recentCalculations->take(8) as $calc)
                                <div class="activity-item">
                                    <div class="activity-left">
                                        <div class="activity-avatar avatar-teal">
                                            {{ strtoupper(substr($calc->deceased_name ?? 'D', 0, 1)) }}
                                        </div>
                                        <div class="activity-details">
                                            <h4>{{ $calc->deceased_name ?? 'Unknown' }}</h4>
                                            <p>
                                                {{ optional($calc->user)->name ?? 'Unknown User' }} 
                                                @if(isset($calc->total_estate))
                                                    • RM {{ number_format($calc->total_estate, 0) }}
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                    <div class="activity-meta">
                                        <span class="activity-time">
                                            @if(isset($calc->created_at) && is_object($calc->created_at))
                ...
                {{-- Recent Instant Estate Sessions --}}
                <div class="activity-card">
                    <div class="activity-card-header">
                        <h3><i class="fas fa-bolt"></i> Recent Instant Sessions</h3>
                        @if(Route::has('admin.instant-estate.index'))
                        <a href="{{ route('admin.instant-estate.index') }}" class="btn btn-sm btn-primary">View All</a>
                        @endif
                    </div>
                    <div class="activity-card-body">
                        @php $recentSessions = $recentSessions ?? collect(); @endphp
                        @if($recentSessions->count() > 0)
                            @foreach($recentSessions->take(8) as $session)
                                @php
                                ...
                                <div class="activity-item">
                                    <div class="activity-left">
                                        <div class="activity-avatar avatar-orange">
                                            {{ strtoupper(substr($deceasedName, 0, 1)) }}
                                        </div>
                                        <div class="activity-details">
                                            <h4>{{ $deceasedName }}</h4>
                                            <p>{{ \Illuminate\Support\Str::limit($session->original_filename ?? 'Unknown file', 50) }}</p>
                                        </div>
                                    </div>
                                    <div class="activity-meta">
                                        <span class="activity-time">
                                            @if(isset($session->created_at) && is_object($session->created_at))
                                                {{ $session->created_at->diffForHumans() }}
                                            @else
                                                Recently
                                            @endif
                                        </span>
                                        <span class="activity-badge {{ $sesBadgeClass }}">{{ $sesStatusLabel }}</span>
                                    </div>
                                </div>
                            @endforeach
                        @else
            ...
            <div class="dashboard-footer">
                <div class="footer-content">
                    <div class="footer-user-info">
                        <div class="footer-user-name">{{ auth()->user()->name ?? 'Admin' }}</div>
                        <div class="footer-user-role">Administrator • {{ now()->format('l, F j, Y') }}</div>
                    </div>
