    ...
    <main class="admin-main">
        <header class="page-header">
            ...
            <div class="header-actions">
                <a href="{{ route('admin.instant-estate.approval.pending') }}" class="btn btn-info">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    Pending Reviews
                </a>
                <a href="{{ route('admin.instant-estate.export') }}" class="btn btn-success">
        ...
        <div class="stats-cards">
            <div class="stats-card"><div class="stats-card-title">Total Sessions</div><div class="stats-card-value">{{ $stats['total'] ?? 0 }}</div></div>
            <div class="stats-card"><div class="stats-card-title">Pending Reviews</div><div class="stats-card-value">{{ $stats['pending_approval'] ?? 0 }}</div></div>
            <div class="stats-card"><div class="stats-card-title">Approved</div><div class="stats-card-value">{{ $stats['approved'] ?? 0 }}</div></div>
        </div>
        ...
        <div class="search-container">
            <div class="search-box">
                ...
                <input type="text" placeholder="Session ID, deceased name, email..." id="search-input" value="{{ request('search') }}">
            </div>
            <div class="filter-group">
                <select id="status-filter" class="filter-select">
                    <option value="">All Statuses</option>
                    @foreach(['uploaded','processing_ocr','ocr_completed','data_confirmed','record_found','no_record','captcha_verified','notification_requested','email_sent','completed','failed','cancelled','expired'] as $status)
        ...
        <div class="table-container">
            @if(isset($sessions) && $sessions->count())
                <div class="table-responsive">
                    <table>
                        ...
                        <tbody id="instant-estate-table-body">
                            ...
                            @endphp
                            <tr class="session-row" data-session-id="{{ $session->session_id }}" data-name="{{ strtolower($session->deceased_name ?? '') }}" data-user="{{ strtolower($userInfo) }}">
                                <td><code>{{ $session->session_id ?: 'N/A' }}</code></td>
                                <td><strong>{{ $session->deceased_name ?: 'N/A' }}</strong></td>
                                <td>{{ $userInfo }}</td>
                                <td>{{ $session->created_at ? $session->created_at->format('d M Y, H:i') : 'N/A' }}</td>
                                <td><span class="status-badge {{ $statusColorClass }}">{{ $session->status_label ?? ucfirst(str_replace('_',' ',$session->status)) }}</span></td>
                                <td>
                                    <div class="action-buttons">
                                        @if($isValidUuid)
                                            <a href="{{ route('admin.instant-estate.show', $session->session_id) }}" class="btn-show">
                                        ...
                                        @if($showEmailActions && $isValidUuid)
                                            @if($canApprove)
                                                <button class="btn-approve approve-notif-btn" data-request-id="{{ $notificationRequestId }}" data-session-name="{{ e($session->deceased_name ?: 'Session') }}">
                                            ...
                                            @endif
                                            @if($canReject)
                                                <button class="btn-reject reject-notif-btn" data-request-id="{{ $notificationRequestId }}" data-session-name="{{ e($session->deceased_name ?: 'Session') }}">
                                            ...
                                            @endif
                                            @if($canSend)
                                                <button class="btn-email send-email-btn" data-request-id="{{ $notificationRequestId }}" data-session-name="{{ e($session->deceased_name ?: 'Session') }}">
                                            ...
                                            @endif
                                            @if($canResend)
                                                <button class="btn-email resend-email-btn" data-request-id="{{ $notificationRequestId }}" data-session-name="{{ e($session->deceased_name ?: 'Session') }}">
                                        ...
                                        @if($isValidUuid)
                                            <button class="btn-delete delete-session-btn" data-id="{{ $session->session_id }}" data-name="{{ e($session->deceased_name ?: 'Session #'.$session->session_id) }}" data-route="{{ route('admin.instant-estate.delete-session', $session->session_id) }}">
                ...
                <div class="pagination-wrapper">
                    <div class="pagination-info">Showing {{ $sessions->firstItem() }}–{{ $sessions->lastItem() }} of {{ $sessions->total() }} sessions</div>
