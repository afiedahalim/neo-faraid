    ...
    <main class="admin-main">
        ...
        <header class="page-header">
            <div class="page-title">
                ...
                <p class="page-subtitle">Deceased: {{ $session->deceased_name ?? 'N/A' }} | ID: {{ $session->session_id }}</p>
            </div>
        </header>
        ...
        <div class="content-block">
            ...
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <div class="info-value">
                        <span class="status-badge status-{{ $session->status }}">
                            {{ $session->status_label ?? ucfirst(str_replace('_', ' ', $session->status)) }}
                        </span>
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Deceased Name</div>
                    <div class="info-value">{{ $session->deceased_name ?: 'N/A' }}</div>
                </div>
                <div class="info-item">
                    ...
                    <div class="info-value">
                        ...
                        @else
                            Guest: {{ $session->guest_name ?: 'Unknown' }}{{ $session->guest_email ? ' ('.$session->guest_email.')' : '' }}
                        @endif
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Uploaded At</div>
                    <div class="info-value">{{ $session->created_at?->format('d M Y, H:i') ?? 'N/A' }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Original Filename</div>
                    <div class="info-value" style="word-break:break-all;">{{ $session->original_filename ?: 'N/A' }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">File Size</div>
                    <div class="info-value">{{ $session->formatted_file_size ?? 'N/A' }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Quality Check</div>
                    <div class="info-value">
                        @if($session->quality_check_passed)
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">{{ $session->ocr_confidence ?? 'N/A' }}%</div>
                </div>
                @if($session->matched_record_type)
                <div class="info-item">
                    <div class="info-label">Matched Record</div>
                    <div class="info-value">
                        {{ ucfirst(str_replace('_', ' ', $session->matched_record_type)) }}
                        (ID: {{ $session->matched_record_id }})
                    </div>
                </div>
                @endif
            </div>
        ...
        @if($session->extracted_data)
            <div class="content-block">
                ...
                <pre class="ocr-data">{{ json_encode($session->extracted_data, JSON_PRETTY_PRINT) }}</pre>
            </div>
        @endif

        <!-- Quality Check Details -->
        @if($session->quality_check_details)
            <div class="content-block">
                <div class="block-title">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <pre class="ocr-data">{{ json_encode($session->quality_check_details, JSON_PRETTY_PRINT) }}</pre>
            </div>
        @endif
        ...
        <div class="action-buttons">
            @if($isValidUuid)
                <button type="button" class="btn btn-delete delete-session-btn"
                        ...
                        data-name="{{ $session->deceased_name ?: 'Session #'.$session->session_id }}"
                        data-route="{{ route('admin.instant-estate.delete-session', $session->session_id) }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18">
