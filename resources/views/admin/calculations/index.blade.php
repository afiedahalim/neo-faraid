    ...
    <main class="admin-main">
        <header class="page-header">
            ...
            <div class="header-actions" style="display: flex; gap: 1rem;">
                <a href="{{ route('admin.calculations.export') }}" class="btn btn-success">
        ...
        <div class="stats-cards">
            <div class="stats-card">
                ...
                <div class="stats-card-value">{{ number_format($stats['total'] ?? 0) }}</div>
            </div>
            <div class="stats-card">
                <div class="stats-card-title">Total Assets Value</div>
                <div class="stats-card-value">RM {{ number_format($stats['total_assets'] ?? 0, 2) }}</div>
            </div>
            <div class="stats-card">
                <div class="stats-card-title">Total Heirs</div>
                <div class="stats-card-value">{{ number_format($stats['total_heirs'] ?? 0) }}</div>
            </div>
            <div class="stats-card">
                <div class="stats-card-title">Avg. Assets/Calc</div>
                <div class="stats-card-value">RM {{ number_format($stats['average_assets'] ?? 0, 2) }}</div>
            </div>
        </div>
                        ...
                        <tbody id="calculation-table-body">
                            @foreach($calculations as $calculation)
                                ...
                                <tr class="calculation-row" 
                                    data-name="{{ strtolower($calculation->deceased_name ?? '') }}" 
                                    data-user="{{ strtolower($calculation->user->name ?? '') }}"
                                    data-id="{{ $calculation->id }}"
                                    data-nric="{{ strtolower($calculation->deceased_nric ?? '') }}">
                                    <td><span style="font-weight: 600; color: var(--primary-color);">#{{ $calculation->id }}</span></td>
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                                            <div class="deceased-avatar">{{ $deceasedInitials }}</div>
                                            <div>
                                                <div style="font-weight: 600; color: var(--primary-color);">{{ $calculation->deceased_name }}</div>
                                                <div style="font-size: 0.85rem; color: var(--text-light);">{{ ucfirst($calculation->deceased_gender) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ $calculation->deceased_nric ?? 'N/A' }}</td>
                                    <td>RM {{ number_format($calculation->total_assets, 2) }}</td>
                                    <td>
                                        <span class="badge badge-info">{{ $calculation->total_heirs ?? 0 }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                                            <div class="user-avatar-table">{{ $userInitials }}</div>
                                            <div>
                                                <div style="font-weight: 600; color: var(--primary-color);">{{ $calculation->user->name ?? 'Unknown' }}</div>
                                                <div style="font-size: 0.85rem; color: var(--text-light);">{{ $calculation->user->email ?? '' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>{{ $calculation->created_at ? $calculation->created_at->format('d M Y') : 'N/A' }}</div>
                                        <small style="color: var(--text-light)">{{ $calculation->created_at ? $calculation->created_at->format('H:i') : '' }}</small>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <a href="{{ route('admin.calculations.show', $calculation->id) }}" 
                                               class="btn-show">
                                            ...
                                            <button type="button" class="btn-delete delete-btn" 
                                                    ...
                                                    data-total-assets="{{ $calculation->total_assets }}">
