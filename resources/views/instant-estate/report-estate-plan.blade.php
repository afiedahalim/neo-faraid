{{-- resources/views/instant-estate/report-estate-plan.blade.php --}}
@extends('layouts.app')

@section('title', 'Inheritance Report - Estate Plan')
...
<header class="report-header">
    ...
    <div class="hero-container">
        ...
        <p class="hero-subtitle">
            Estate Plan for <strong>{{ $deceasedName }}</strong>
        </p>
    </div>
</header>
...
<div class="glass-container">
        ...
        <div class="card-body">
            <div class="grid-3" style="margin-bottom: 1.5rem;">
                <div style="background: var(--white); padding: 1.75rem; border-radius: var(--border-radius-md); border: 1px solid var(--gray-200); text-align: center;">
                    ...
                    <div style="font-size: 2rem; font-weight: 800; color: var(--success-color);">RM {{ number_format($totalAssets, 2) }}</div>
                </div>
                <div style="background: var(--white); padding: 1.75rem; border-radius: var(--border-radius-md); border: 1px solid var(--gray-200); text-align: center;">
                    ...
                    <div style="font-size: 2rem; font-weight: 800; color: var(--danger-color);">RM {{ number_format($totalDebts, 2) }}</div>
                </div>
                <div style="background: var(--white); padding: 1.75rem; border-radius: var(--border-radius-md); border: 1px solid var(--gray-200); text-align: center;">
                    ...
                    <div style="font-size: 2rem; font-weight: 800; color: var(--primary-color);">RM {{ number_format($netEstate, 2) }}</div>
                </div>
            </div>
            ...
            <div class="grid-2" style="margin-top: 1rem;">
                <div style="background: linear-gradient(135deg, var(--white) 0%, var(--warning-light) 100%); padding: 1.5rem; border-radius: var(--border-radius-md); border: 1px solid rgba(255, 193, 7, 0.3); text-align: center;">
                    ...
                    <div style="font-size: 1.75rem; font-weight: 800; color: var(--warning-dark);">RM {{ number_format($wasiyyahAmount, 2) }}</div>
                    <div style="font-size: 0.8125rem; color: var(--gray-600); margin-top: 0.25rem;">{{ number_format($effectiveWasiyyahPct, 2) }}% of net estate</div>
                </div>
                <div style="background: linear-gradient(135deg, var(--white) 0%, var(--info-light) 100%); padding: 1.5rem; border-radius: var(--border-radius-md); border: 1px solid rgba(23, 162, 184, 0.3); text-align: center;">
                    ...
                    <div style="font-size: 1.75rem; font-weight: 800; color: var(--info-color);">RM {{ number_format($remainingForHeirs, 2) }}</div>
            ...
            <div style="margin-top: 1.5rem;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.875rem;">
                    <span><strong>Wasiyyah Limit:</strong> 1/3 of Net Estate (RM {{ number_format(($netEstate * 33.33) / 100, 2) }})</span>
                    <span><strong>Current:</strong> {{ number_format($totalWasiyyahPct, 2) }}%</span>
                </div>
                <div class="wasiyyah-limit-bar">
                    <div class="wasiyyah-limit-fill {{ $totalWasiyyahPct <= $maxWasiyyahPct ? 'safe' : '' }}" style="width: {{ min(100, ($totalWasiyyahPct / $maxWasiyyahPct) * 100) }}%;"></div>
                </div>
            </div>
            @endif
        </div>
    ...
    <div class="glass-card">
        <div class="card-header">
            ...
            <span class="card-badge">{{ count($assets) }} Asset{{ count($assets) !== 1 ? 's' : '' }}</span>
        </div>
        <div class="card-body">
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        @foreach($assets as $asset)
                            ...
                            <tr>
                                <td>
                                    <span class="badge badge-{{ $badge }}">{{ $assetName }}</span>
                                </td>
                                <td>{{ $badge === 'real-estate' ? 'Real Estate' : ($badge === 'financial' ? 'Financial' : ($badge === 'investment' ? 'Investment' : ($badge === 'digital' ? 'Digital' : 'Other'))) }}</td>
                                <td style="text-align: right; font-weight: 600;">RM {{ number_format($assetValue, 2) }}</td>
                                <td style="text-align: center;">{{ number_format($assetOwnership, 0) }}%</td>
                                <td style="text-align: right; font-weight: 600; color: var(--success-color);">RM {{ number_format($assetOwnedValue, 2) }}</td>
                    ...
                    <tfoot>
                        <tr style="background: var(--gray-100);">
                            ...
                            <td style="text-align: right;"><strong style="color: var(--success-color);">RM {{ number_format($totalAssets, 2) }}</strong></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
    ...
    <div class="glass-card">
        <div class="card-header">
            ...
            <span class="card-badge">{{ count($debts) }} Debt{{ count($debts) !== 1 ? 's' : '' }}</span>
        </div>
        <div class="card-body">
            <div class="results-container">
                <table class="results-table">
                    ...
                    <tbody>
                        @foreach($debts as $debt)
                            ...
                            @endphp
                            <tr>
                                <td><strong>{{ $creditorName }}</strong></td>
                                <td><span class="badge badge-{{ $dBadge }}">{{ $debtType }}</span></td>
                                <td>{{ $dBadge === 'secured' ? 'Secured Debt' : ($dBadge === 'religious' ? 'Religious Obligation' : 'Unsecured Debt') }}</td>
                                <td>{{ $debtDescription }}</td>
                                <td style="text-align: right; font-weight: 600; color: var(--danger-color);">RM {{ number_format($debtAmount, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr style="background: var(--gray-100);">
                            ...
                            <td style="text-align: right;"><strong style="color: var(--danger-color);">RM {{ number_format($totalDebts, 2) }}</strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    <div class="glass-card">
        <div class="card-header">
            ...
            <span class="card-badge" style="background: var(--warning-light); color: #b76e00;">{{ count($wasiyyah) }} Beneficiar{{ count($wasiyyah) !== 1 ? 'ies' : 'y' }}</span>
        </div>
        <div class="card-body">
            <div class="info-card" style="border-left-color: var(--warning-color); background: linear-gradient(135deg, var(--white) 0%, var(--warning-light) 100%);">
                ...
                <p>Wasiyyah is limited to <strong>1/3 (33.33%)</strong> of the net estate as per Islamic law. Current usage: <strong style="color: {{ $totalWasiyyahPct > $maxWasiyyahPct ? 'var(--danger-color)' : 'var(--success-color)' }};">{{ number_format($totalWasiyyahPct, 2) }}%</strong></p>
            </div>
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        @foreach($wasiyyah as $item)
                            ...
                            <tr>
                                ...
                                <td>{{ $beneficiaryNric }}</td>
                                <td>{{ $beneficiaryRelationship }}</td>
                                <td style="text-align: center;">{{ number_format($requestedPct, 2) }}%</td>
                                <td style="text-align: right; font-weight: 600; color: var(--warning-dark);">RM {{ number_format($wasAmount, 2) }}</td>
                                <td>
                                    @if($isCharity)
                    ...
                    <tfoot>
                        <tr style="background: var(--gray-100);">
                            ...
                            <td style="text-align: center;"><strong>{{ number_format($totalWasiyyahPct, 2) }}%</strong></td>
                            <td style="text-align: right;"><strong style="color: var(--warning-dark);">RM {{ number_format($wasiyyahAmount, 2) }}</strong></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
    ...
    <div class="glass-card">
        <div class="card-header">
            ...
            <span class="card-badge badge-heir">{{ count($heirs) }} Heir{{ count($heirs) !== 1 ? 's' : '' }}</span>
        </div>
        <div class="card-body">
            <div class="info-card">
                <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        @foreach($heirs as $heir)
                            ...
                            @endphp
                            <tr>
                                <td><span class="badge badge-heir">Heir</span> <strong>{{ $heirName }}</strong></td>
                                <td>{{ $heirNric }}</td>
                                <td>{{ ucfirst(str_replace('_', ' ', $heirRelationship)) }}</td>
                                <td style="text-align: center;">{{ $heirShareFraction ?: number_format($heirSharePercentage, 2) . '%' }}</td>
                                <td style="text-align: center; font-weight: 600; color: var(--primary-color);">{{ number_format($heirSharePercentage, 2) }}%</td>
                                <td style="text-align: right; font-weight: 700; color: var(--primary-color);">RM {{ number_format($heirShareAmount, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr style="background: var(--primary-light);">
                            ...
                            <td style="text-align: center;"><strong style="color: {{ abs($totalHeirPct - 100) < 0.01 ? 'var(--success-color)' : 'var(--danger-color)' }};">{{ number_format($totalHeirPct, 2) }}%</strong></td>
                            <td style="text-align: right;"><strong style="color: var(--primary-color);">RM {{ number_format($remainingForHeirs, 2) }}</strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div class="card-body">
            <div class="grid-2">
                <div>
                    ...
                    <span style="font-size: 1.125rem; font-weight: 600;">{{ $trusteeName }}</span></p>
                    
                    <p style="margin-bottom: 0.75rem;"><strong style="color: var(--gray-600);">Relationship:</strong><br>
                    <span style="font-weight: 500;">{{ $trusteeRelationship }}</span></p>
                    
                    @if($trusteeNric)
                    <p style="margin-bottom: 0.75rem;"><strong style="color: var(--gray-600);">NRIC/Passport:</strong><br>
                    <span style="font-weight: 500;">{{ $trusteeNric }}</span></p>
                    @endif
                </div>
                <div>
                    <p style="margin-bottom: 0.75rem;"><strong style="color: var(--gray-600);">Contact Email:</strong><br>
                    <span style="font-weight: 500;">{{ $trusteeEmail ?? 'N/A' }}</span></p>
                    
                    @if($trusteePhone)
                    <p style="margin-bottom: 0.75rem;"><strong style="color: var(--gray-600);">Contact Phone:</strong><br>
                    <span style="font-weight: 500;">{{ $trusteePhone }}</span></p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div style="text-align: center; padding: 2rem 0; color: var(--gray-500); font-size: 0.8125rem;">
        ...
        <p style="margin-bottom: 0.25rem;">Generated on: <strong>{{ now()->format('d F Y, h:i:s A') }}</strong></p>
        <p style="margin-bottom: 0;">Document ID: <strong style="color: var(--primary-color);">{{ $documentId }}</strong></p>
    </div>
</div>
...
<div id="pdfContent" style="display:none;">
    <div style="padding:20px;font-family:'Poppins',sans-serif;max-width:800px;margin:0 auto;">
        <div style="text-align:center;margin-bottom:20px;border-bottom:2px solid #1a5fb4;padding-bottom:15px;">
            ...
            <p style="color:#64748b;font-size:12px;margin:0;">Reference: {{ $documentId }} | Status: {{ ucfirst($estateStatus) }} | Generated: {{ now()->format('d M Y') }}</p>
        </div>

        <!-- Deceased Information -->
        <div class="pdf-section-title">Deceased Information</div>
        <table class="pdf-info-table">
            <tr><td><strong>Name</strong></td><td>{{ $deceasedName }}</td></tr>
            <tr><td><strong>NRIC/Passport</strong></td><td>{{ $deceasedNric }}</td></tr>
            <tr><td><strong>Gender</strong></td><td>{{ ucfirst($deceasedGender) }}</td></tr>
            <tr><td><strong>Date of Birth</strong></td><td>{{ $deceasedDob }}</td></tr>
            <tr><td><strong>Date of Death</strong></td><td>{{ $dateOfDeath }}</td></tr>
            <tr><td><strong>Address</strong></td><td>{{ $deceasedAddress }}</td></tr>
        </table>
        ...
        <div style="display:flex;gap:15px;margin-bottom:15px;">
            ...
            <div class="pdf-info-card" style="flex:1;text-align:center;"><strong>Net Estate</strong><br><span style="color:#1a5fb4;font-size:16px;">{{ $formattedNetEstate }}</span></div>
        </div>
        ...
        @if(count($assetsList) > 0)
            ...
            <table class="pdf-info-table">
                ...
                <tbody>
                    ...
                    <tr><td>{{ $asset['name'] }}</td><td>RM {{ number_format($asset['value'], 2) }}</td><td>{{ $asset['ownership_percentage'] }}%</td></tr>
                    @endforeach
        ...
        @if(count($debtsList) > 0)
            ...
            <table class="pdf-info-table">
                ...
                <tbody>
                    ...
                    <tr><td>{{ $debt['creditor_name'] }}</td><td>{{ $debt['debt_type'] }}</td><td>RM {{ number_format($debt['amount'], 2) }}</td></tr>
                    @endforeach
            ...
            <table class="pdf-info-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format($heir['percentage'], 2) }}%</td>
            ...
            <table class="pdf-info-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>RM {{ number_format($wasAmount, 2) }}</td>
                    </tr>
                    @endforeach
                    <tr style="background:#f1f5f9;font-weight:700;">
        ...
        @if($trusteeName)
            ...
            <table class="pdf-info-table">
                ...
                <tr><td><strong>Relationship</strong></td><td>{{ $trusteeRelationship }}</td></tr>
                @if($trusteeNric)<tr><td><strong>NRIC/Passport</strong></td><td>{{ $trusteeNric }}</td></tr>@endif
                <tr><td><strong>Email</strong></td><td>{{ $trusteeEmail ?? 'Not provided' }}</td></tr>
                @if($trusteePhone)<tr><td><strong>Phone</strong></td><td>{{ $trusteePhone }}</td></tr>@endif
            </table>
        @endif

        <div style="margin-top:20px;padding-top:10px;border-top:1px solid #e2e8f0;font-size:10px;color:#94a3b8;text-align:center;">
            <p>This is an official estate distribution statement. Generated on {{ now()->format('d F Y, h:i A') }} | Document ID: {{ $documentId }}</p>
...
function downloadPDF() {
    ...
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>Estate Distribution Statement - {{ $deceasedName }}</title>
            <link rel="preconnect" href="https://fonts.googleapis.com">
            <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
            <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
            <style>
