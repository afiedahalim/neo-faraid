...
@section('title', 'Instant Estate History')
    ...
    <div class="glass-card">
        <div class="card-header">
            <div class="card-header-left">
                ...
                <span class="card-badge">{{ $sessions instanceof \Illuminate\Pagination\LengthAwarePaginator ? $sessions->total() : $sessions->count() }} Total Records</span>
            </div>
            <a href="{{ route('instant-estate.index') }}" class="btn btn-primary">
                        ...
                        <tr>
                            <td>
                                <code style="background: var(--gray-100); padding: 0.25rem 0.5rem; border-radius: 6px; font-size: 0.75rem;">
                                    {{ $safeSessionId ? substr($safeSessionId, 0, 8) . '...' : 'N/A' }}
                                </code>
                            </td>
                            <td style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $session->original_filename }}">
                                {{ Str::limit($session->original_filename ?? 'Unknown', 30) }}
                            </td>
                            <td>
                                @php
                                    $statusColors = [
                            ...
                            <!-- Admin Status Column -->
                            <td>
                                @if($session->admin_status_label)
                                    <span class="badge badge-{{ $session->admin_status_color ?? 'secondary' }}">
                                        {{ $session->admin_status_label }}
                                    </span>
                                @else
                            ...
                            <td>
                                {{ $session->extracted_data['deceased_name'] ?? $session->deceased_name ?? 'N/A' }}
                                @if(!empty($session->extracted_data['deceased_nric']) || !empty($session->deceased_nric))
                                    <br>
                                    <small style="color: var(--gray-500); font-size: 0.7rem;">
                                        {{ $session->extracted_data['deceased_nric'] ?? $session->deceased_nric }}
                                    </small>
                                @endif
                            </td>
                            <td>
                                <span title="{{ $session->created_at ? $session->created_at->format('Y-m-d H:i:s') : '' }}">
                            ...
                            <td>
                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
                                    @if($session->calculation_id)
                                        <a href="{{ route('calculator.show', $session->calculation_id) }}" 
                                    ...
                                    @elseif(in_array($session->status, ['ocr_completed', 'auto_filled']) && $safeSessionId)
                                        <a href="{{ route('instant-estate.view-session', $safeSessionId) }}" 
                                           class="btn btn-outline-primary btn-sm">
                                            <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {{-- Send Report Link Button (only if approved AND valid UUID) --}}
                                    @if($session->admin_status === 'approved' && $isValidUuid && $safeSessionId)
                                        <button type="button" 
                                                class="btn btn-success btn-sm send-report-btn"
                                                data-session-id="{{ $safeSessionId }}"
                                                data-deceased-name="{{ $session->deceased_name ?? $session->extracted_data['deceased_name'] ?? 'the deceased' }}"
                                                data-default-email="{{ $session->contact_email ?? $session->guest_email ?? Auth::user()->email ?? '' }}">
                                            <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {{-- DELETE BUTTON – only one per row, tied to the single modal --}}
                                    @if(!$session->calculation_id && $isValidUuid && $safeSessionId)
                                        <button type="button" 
                                                class="btn btn-outline-danger btn-sm delete-btn"
                                                data-session-id="{{ $safeSessionId }}"
                                                data-filename="{{ $session->original_filename ?? 'Unknown' }}"
                                                data-delete-url="{{ route('instant-estate.delete-session', $safeSessionId) }}">
                                            <svg class="btn-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                            </svg>
                                            Delete
