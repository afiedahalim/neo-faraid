...
<div class="glass-container">
        ...
        <div class="card-body">
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value">{{ number_format((int)($stats['total_calculations'] ?? 0)) }}</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value">RM {{ number_format((float)($stats['total_assets'] ?? 0), 2) }}</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value">{{ number_format((int)($stats['total_heirs'] ?? 0)) }}</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <div class="stat-value">RM {{ number_format((float)($stats['average_assets'] ?? 0), 2) }}</div>
            ...
            <!-- Latest Calculation -->
            @if(isset($stats['latest_calculation']) && $stats['latest_calculation'])
            <div class="latest-card" style="margin: 0; border-top: 1px solid var(--gray-200); border-radius: 0;">
                ...
                <div class="latest-content">
                    <div class="latest-info">
                        <div class="latest-name">{{ $stats['latest_calculation']->deceased_name }}</div>
                        <div class="latest-date">{{ $stats['latest_calculation']->created_at->format('d M Y, h:i A') }}</div>
                    </div>
                    <a href="{{ route('calculator.show', $stats['latest_calculation']->id) }}" class="btn-view">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    <div class="glass-card">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('calculator.history') }}" class="filter-form">
                ...
                <div class="btn-group-modern" style="margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid var(--light-border);">
                    ...
                    <a href="{{ route('calculator.history') }}" class="btn btn-secondary">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width: 20px; height: 20px;">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                        Clear Filters
    ...
    <div class="glass-card">
        <div class="card-header">
            ...
            <span class="card-badge">{{ $calculations->count() ?? 0 }} Records</span>
        </div>
        ...
        <div class="card-body">
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="calculation-info">
                                    ...
                                    <div class="calculation-details">
                                        <h4>{{ $calculation->deceased_name }}</h4>
                                        <p>ID: {{ $calculation->id }} • {{ ucfirst($calculation->deceased_gender ?? 'N/A') }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="date-info">
                                    <span class="date">{{ $calculation->created_at->format('d M Y') }}</span>
                                    <span class="time">{{ $calculation->created_at->format('h:i A') }}</span>
                                </div>
                            </td>
                            
                            <td>
                                <div class="amount-info">
                                    <span class="amount">RM {{ number_format($calculation->total_assets, 2) }}</span>
                                    <span class="net-amount">Net: RM {{ number_format($calculation->net_assets ?? 0, 2) }}</span>
                                </div>
                            </td>
                            
                            <td>
                                <div class="heirs-info">
                                    <span class="heirs-count">{{ $calculation->total_heirs ?? 0 }}</span>
                                    <span>heir{{ ($calculation->total_heirs ?? 0) > 1 ? 's' : '' }}</span>
                                </div>
                            </td>
                            
                            <td>
                                <span class="badge">Scenario {{ $calculation->scenario_number ?? 1 }}</span>
                            </td>
                            
                            <td class="text-end">
                                <div class="action-buttons">
                                    <a href="{{ route('calculator.show', $calculation->id) }}" 
                                    ...
                                    <a href="{{ route('calculator.edit', $calculation->id) }}" 
                                    ...
                                    <button type="button" 
                                            ...
                                            data-deceased-name="{{ $calculation->deceased_name }}"
                                    ...
                                    <form action="{{ route('calculator.destroy', $calculation->id) }}" 
                                          method="POST" 
                                          id="delete-form-{{ $calculation->id }}"
                                          style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
            ...
            <div class="pagination-section">
                <div class="pagination-info">
                    Showing {{ $calculations->firstItem() }} to {{ $calculations->lastItem() }} 
                    of {{ $calculations->total() }} calculations
                </div>
                
                <nav class="pagination-nav">
                    @if ($calculations->onFirstPage())
                    <span class="pagination-disabled">Previous</span>
                    @else
                    <a href="{{ $calculations->previousPageUrl() }}" class="pagination-link">Previous</a>
                    @endif
                    
                    @foreach ($calculations->links()->elements[0] as $page => $url)
                    <a href="{{ $url }}" class="pagination-number {{ $page == $calculations->currentPage() ? 'active' : '' }}">
                        {{ $page }}
                    </a>
                    @endforeach
                    
                    @if ($calculations->hasMorePages())
                    <a href="{{ $calculations->nextPageUrl() }}" class="pagination-link">Next</a>
                    @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if(session('success'))
    ModernAlert.success("{{ session('success') }}");
    @endif
    
    @if(session('error'))
    ModernAlert.error("{{ session('error') }}");
    @endif
    
    @if(session('warning'))
    ModernAlert.warning("{{ session('warning') }}");
    @endif
    
    @if(session('info'))
    ModernAlert.info("{{ session('info') }}");
    @endif
